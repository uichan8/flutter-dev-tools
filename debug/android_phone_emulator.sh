#!/bin/bash
# Android 폰 에뮬레이터 디버그 빌드 및 실행 스크립트
# 사용법: ./scripts/debug/android_phone_emulator.sh
...
if ! flutter devices | grep -q android; then
    ...
    echo "에뮬레이터 실행 중..."
    echo "설정된 에뮬레이터: $ANDROID_AVD_PHONE"
    "$EMULATOR_CMD" -avd "$ANDROID_AVD_PHONE" &
    EMULATOR_PID=$!
    echo "에뮬레이터 부팅 대기 중..."
